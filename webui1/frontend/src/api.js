...
export async function queryWords(p) {
  ...
  const r = await axios.post(`${API}/api/query`, fd);
...
export async function generateBatchName(p) {
  ...
  const r = await axios.post(`${API}/api/generate_batch_name`, fd);
  return r.data?.batch || '';
}
...
export async function commitBatch(edited, batch) {
  ...
  const r = await axios.post(`${API}/api/commit`, fd);
  return r.data;
}
...
export async function getReminders() {
  const r = await axios.get(`${API}/api/reminders`);
  return r.data?.words || [];
}

export async function getReminderResults() {
  const r = await axios.get(`${API}/api/reminders/results`);
  return r.data?.results || [];
}
...
export async function updateReminders(action, words) {
  ...
  const r = await axios.post(`${API}/api/reminders`, fd);
  return r.data;
}
